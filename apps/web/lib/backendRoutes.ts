...
export const backendRoutes = {
  ...
  auth: {
    sendOtp: (body: { phone: string }) => apiFetch("/auth/otp/send", { method: "POST", auth: "omit", body }),
    verifyOtp: (body: { phone: string; code: string }) =>
      apiFetch("/auth/otp/verify", { method: "POST", auth: "omit", body }),
    register: (body: Record<string, unknown>) =>
      apiFetch("/auth/register", { method: "POST", auth: "omit", body }),
    login: (body: { phone: string; password: string }) =>
      apiFetch("/auth/login", { method: "POST", auth: "omit", body }),
    refresh: () => apiFetch("/auth/token/refresh", { method: "POST", auth: "omit", body: {} }),
  ...
  likes: {
    create: (body: Record<string, unknown>) => apiFetch("/likes", { method: "POST", body }),
    incoming: () => apiFetch("/likes/incoming")
  },
  matches: {
    list: () => apiFetch("/matches"),
    respondConsent: (body: Record<string, unknown>) =>
      apiFetch("/consent/respond", { method: "POST", body }),
    phoneUnlock: (matchId: string) => apiFetch(`/phone-unlock/${matchId}`)
  },
  reports: {
    create: (body: Record<string, unknown>) => apiFetch("/reports", { method: "POST", body })
  },
  refunds: {
    request: (body: Record<string, unknown>) => apiFetch("/refunds/request", { method: "POST", body }),
  ...
  admin: {
    ...
    startVerificationRequest: (id: string, meetUrl: string) =>
      apiFetch(`/admin/verification-requests/${id}/start`, { method: "POST", body: { meetUrl } }),
    ...
    rejectVerificationRequest: (id: string, reason: string) =>
      apiFetch(`/admin/verification-requests/${id}/reject`, { method: "POST", body: { reason } }),
    setVerificationMeetLink: (userId: string, meetUrl: string) =>
      apiFetch(`/admin/verifications/${userId}/meet-link`, { method: "POST", body: { meetUrl } }),
    approveVerificationForUser: (userId: string, reason?: string) =>
      apiFetch(`/admin/verifications/${userId}/approve`, { method: "POST", body: { reason } }),
    rejectVerificationForUser: (userId: string, reason: string) =>
      apiFetch(`/admin/verifications/${userId}/reject`, { method: "POST", body: { reason } }),
    shiftPaymentDate: (body: { userId: string; daysBack: number }) =>
      apiFetch("/admin/dev/shift-payment-date", { method: "POST", body })
  },
  verification: {
  ...
  payments: {
    ...
    validateCoupon: (code: string) => apiFetch("/payments/coupon/validate", { method: "POST", body: { code } }),
    ...
    confirmMock: (paymentIntentId: string) =>
      apiFetch("/payments/mock/confirm", { method: "POST", body: { paymentIntentId } })
  },
  notifications: {
