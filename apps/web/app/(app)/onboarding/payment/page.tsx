...
export default function PaymentStep() {
  ...
  const formattedPlans = useMemo(
    () =>
      overview?.plans.map((plan) => ({ ...plan, label: PLAN_COPY[plan.plan].label, price: PLAN_COPY[plan.plan].price, perks: ["Private Club Access", "Curated Discovery", "Premium Member Support"] })) ?? [],
    [overview]
  );
  ...
  if (loading) return <div className="flex h-full items-center justify-center text-sm text-foreground/50">Loading membership plans…</div>;
    ...
    <div className="flex flex-col h-full px-8 pb-[calc(env(safe-area-inset-bottom,0px)+32px)]">
      ...
      <div className="flex-none grid grid-cols-1 gap-4 mb-8">
        {formattedPlans.map((tier) => {
          ...
          return (
            <motion.button key={tier.plan} whileTap={{ scale: 0.98 }} onClick={() => setSelectedTier(tier.plan)} className={`relative p-5 rounded-2xl border text-left ${active ? "bg-primary/10 border-primary" : "bg-foreground/[0.02] border-foreground/8"}`}>
              ...
              {active && <div className="absolute bottom-3 right-3 w-5 h-5 rounded-full bg-primary flex items-center justify-center"><Check size={11} className="text-background" strokeWidth={2.5} /></div>}
            </motion.button>
          );
        })}
      </div>
      <form onSubmit={handlePayment} className="flex flex-col flex-1 gap-6">
        <div className="mt-auto space-y-3">
          ...
          {allowTestBypass && (
            <button type="button" onClick={() => void handleMockPayment()} className="w-full text-xs uppercase tracking-[0.2em] text-amber-300 hover:text-amber-200">
