...
import { Loader2, MapPin, Phone, UserMinus, Video, Link2, ShieldCheck, RefreshCcw } from "lucide-react";
...
import { getOfflineMeet, getOnlineMeet, getPhoneUnlock, getSocialExchange, initiateMatchInteractionRequest, unmatch } from "@/lib/matches";
import { ApiError } from "@/lib/api";

type PendingAction = `${string}:${MatchRequestType}` | `unmatch:${string}`;
    ...
    try {
      ...
      if (response.ready) {
        ...
        if (type === "OFFLINE_MEET") {
          await getOfflineMeet(matchId).catch(() => null);
        }
        if (type === "ONLINE_MEET") {
      ...
      setFeedback(response.ready
        ? "Request accepted by both members. Concierge will coordinate next steps via Alerts."
  ...
  return (
    <div className="w-full px-6 md:px-8 pt-8 pb-24">
      ...
      {!matchesQuery.isRefreshing && matchesQuery.data === undefined ? (
        <div className="rounded-3xl border border-border/40 bg-foreground/[0.02] p-6 text-center">
          <p className="text-sm text-foreground/70">We could not load your matches right now.</p>
          <button
            onClick={() => void matchesQuery.refresh(true)}
            className="mt-4 inline-flex items-center gap-2 rounded-full border border-primary/30 px-4 py-2 text-[11px] uppercase tracking-[0.2em] text-primary"
          >
        ...
        {matches.map((match) => (
          <article key={match.id} className="rounded-3xl border border-border/40 bg-foreground/[0.02] overflow-hidden">
            ...
            <div className="p-4">
              ...
              <div className="mt-4 grid grid-cols-1 gap-2">
                {interactionMeta.map(({ type, label, icon: Icon }) => {
                  ...
                  return (
                    <button
                      key={type}
                      onClick={() => void runInteraction(match.id, type)}
                      disabled={disabled}
                      className="flex items-center justify-between rounded-xl border border-border/40 bg-background/60 px-3 py-3 disabled:opacity-50"
                    >
              ...
              <button
                onClick={() => void runUnmatch(match.id)}
                disabled={pendingAction === `unmatch:${match.id}`}
                className="mt-4 w-full inline-flex items-center justify-center gap-2 rounded-full border border-primary/25 px-4 py-2 text-[11px] uppercase tracking-[0.2em] text-foreground/70 disabled:opacity-50"
              >
