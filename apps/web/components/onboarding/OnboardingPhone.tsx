...
export default function OnboardingPhone({ onNext, isLoading = false }: OnboardingPhoneProps) {
  const [phone, setPhone] = useState(() => getOnboardingData().phone || '');
  ...
  const getDigitsOnly = (phoneStr: string) => phoneStr.replace(/\D/g, '');
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          {isSubmitting || isLoading ? 'Sending Code...' : 'Send Code'}
        </button>
      </form>
    </div>
  );
